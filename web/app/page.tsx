                ...
                <section className="tab-panel audio-lab-panel">
                  ...
                  <section className="audio-export-section">
                    ...
                    <div className="audio-export-grid">
                      {artifacts.filter((artifact) => /audacity|audio_(?:mono|left|right|stereo)/i.test(artifactName(artifact))).map((artifact) => <a key={artifactId(artifact)} href={normalizeUrl(artifact.download_url) || `${API_BASE}/api/jobs/${jobId}/artifacts/${artifactId(artifact)}/download`}><span>{artifactMediaType(artifact).startsWith('audio/') ? '≋' : '⌁'}</span><div><strong>{artifactName(artifact)}</strong><small>{formatBytes(artifactSize(artifact))} · import into Audacity</small></div><em>↓</em></a>)}
                  ...
                  <div className="artifact-workspace">
                    <div className="artifact-table" role="table" aria-label="Recovered artifacts">
                      ...
                      {filteredArtifacts.map((artifact, index) => {
                        ...
                        return <div className={`artifact-row ${artifactId(selectedArtifact || {}) === id ? 'selected' : ''}`} role="row" key={id || index} style={{ '--depth': Math.min(artifactDepth(artifact), 4) } as CSSProperties}><span><i>{artifact.parent_id || artifactDepth(artifact) ? '└' : '◆'}</i><button className="artifact-open" onClick={() => setSelectedArtifact(artifact)}><strong>{artifactName(artifact)}</strong><small>{artifactOrigin(artifact)}</small></button></span><span><em>{artifactMediaType(artifact)}</em></span><span>{formatBytes(artifactSize(artifact))}</span><span className="mono">{artifact.sha256?.slice(0, 12) || '—'}</span><a href={download} aria-label={`Download ${artifactName(artifact)}`}>↓</a></div>;
                      })}
