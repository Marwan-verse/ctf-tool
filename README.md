...
- Wireshark `capinfos`/TShark summaries, protocol fields, USB HID decoding, isolated HTTP/SMB/TFTP/IMF object extraction, and `pcapfix` capture repair
...
Deep capture scans recover IPv4/IPv6/TCP/UDP/ICMP flows across classic PCAP and multi-section PCAPNG (including IDB timestamp resolution, EPB/SPB/PB blocks, comments, and decryption-secret blocks). The bounded native pass reassembles out-of-order TCP spans with retransmission/overlap clipping, joins UDP conversations, parses DNS questions/TXT/CNAME/A/AAAA data, reconstructs TFTP RRQ/WRQ/DATA objects, and tests timestamp-ordered per-packet Base64, hex/Base32, URL, compression, constant-XOR/byte-shift, source-port, TTL/IP-ID/ICMP/TCP-ACK covert channels. Character-spaced payload flags are preserved as evidence and scanned again after inter-token whitespace normalization. Captures can concatenate Base64 fragments and try repeating XOR keys derived from decimal IMSI windows, which covers rogue-cell exfiltration patterns without requiring Wireshark. Printable hex packet payloads and non-IP Ethernet data are turned back into candidate text or files, ordinary eight-byte USB boot-keyboard reports are decoded (including shift and backspace), and bounded HTTP/SMB/TFTP/IMF exported objects are retained for recursive flag scanning. The optional Wireshark adapters expose full-capture forensic fields and `pcapfix` writes a separate repaired copy; neither path overwrites the source. Deep disk scans derive MBR/GPT sector starts, enumerate each bounded partition with Sleuth Kit, include deleted names, and recover allocated/unallocated files into isolated temporary output before applying the normal artifact limits. Office and RTF scans expose decoded VBA and embedded objects without launching Microsoft Office. Volatility runs with `--offline`, so symbol availability is reported honestly and an analysis never downloads data implicitly.
