...
Deep capture scans recover IPv4/IPv6/TCP/UDP/ICMP flows across classic PCAP and multi-section PCAPNG (including interface labels, timestamp resolution, EPB/SPB/PB blocks, comments, name-resolution, and decryption-secret blocks). A dedicated Traffic tab exposes a bounded packet list, display filters, packet layers and hex bytes, protocol hierarchy, endpoints, conversations, stream summaries, DNS/HTTP/TLS records, recovered objects, and expert-style events. The native pass reassembles out-of-order TCP spans with retransmission/overlap clipping, joins UDP and ICMP conversations, parses ARP, DHCP option clues, CoAP paths/payloads, DNS questions/TXT/CNAME/A/AAAA data, cleartext HTTP/2 DATA streams, and TFTP RRQ/WRQ/DATA objects, then tests timestamp-ordered per-packet Base64, hex/Base32, URL, compression, constant-XOR/byte-shift, source/destination-address, source-port, TTL/IP-ID/ICMP/TCP header covert channels. It also packs TCP/IP flag bit planes in both bit orders and directions, clusters inter-packet timing deltas, scans direct millisecond/centisecond/microsecond timing bytes, removes repeated ICMP padding, and reconstructs payloads selected by IPv4 reserved/DF or TCP PSH/RST/URG flags. SocketCAN frames gain arbitration-ID lanes and ISO-TP reassembly; MQTT PUBLISH topics, WebSocket frames, Modbus register data, BitTorrent DHT info-hashes, RTP DTMF events, G.711 WAV streams, and cleartext FTP/Telnet/mail authentication commands are surfaced safely. Character-spaced payload flags are preserved as evidence and scanned again after inter-token whitespace normalization. Captures can concatenate Base64 fragments and try repeating XOR keys derived from decimal IMSI windows, which covers rogue-cell exfiltration patterns without requiring Wireshark. Printable hex packet payloads and non-IP Ethernet data are turned back into candidate text or files; TShark USB HID analysis decodes keyboards and can render relative mouse motion as SVG. Bounded HTTP/FTP-DATA/SMB/TFTP/IMF/DICOM exports and an HTTP/2 Content-Range reassembler are retained for recursive flag scanning. Optional adapters use Wireshark's own dissectors for bounded JSON packet trees, statistics, Expert Info, credentials, RTP, and object exports; `tcpflow`, `hcxpcapngtool`, and `pcapfix` produce isolated child artifacts and never overwrite the source. Deep disk scans derive MBR/GPT sector starts, enumerate each bounded partition with Sleuth Kit, include deleted names, and recover allocated/unallocated files into isolated temporary output before applying the normal artifact limits. Office and RTF scans expose decoded VBA and embedded objects without launching Microsoft Office. Volatility runs with `--offline`, so symbol availability is reported honestly and an analysis never downloads data implicitly.
