...
class AnalysisEngine:
    """Synchronous, local-first image and audio forensics orchestrator.
    ...
    def run(
        ...
        if str(analysis_options.get("evidence_type") or "auto") == "audio" or probe_kind in AUDIO_KINDS:
        ...
        *,
        ...
        try:
            ...
            for order, item in enumerate(audio_visuals):
                ...
                if artifact_id:
                    ...
                    visual_views.append({
                        "id": f"visual-{len(visual_views) + 1:04d}", "artifact_id": artifact_id,
                        "title": display_text(item.get("title", item["label"]), 200),
                        "category": "audio-spectrogram" if "spectrogram" in item["label"] else "audio-waveform",
            ...
            if not analysis_options.get("external_tools", True):
                ...
                if not analysis_options.get("audio_audacity_bundle", True):
